...
const consentPersonalData: TemplateDoc = {
  title: "Согласие на обработку персональных данных",
  heading: "СОГЛАСИЕ на обработку персональных данных",
  fields: [
    { key: "fio", label: "ФИО клиента", type: "text", placeholder: "Иванова Анна Петровна" },
    { key: "birth", label: "Дата рождения", type: "date", optional: true },
    {
      key: "identMode",
      label: "Как указать личность в документе",
      type: "radio",
    ...
    { key: "passport", label: "Паспорт (серия, номер)", type: "text", placeholder: "4510 123456", optional: true },
    { key: "address", label: "Адрес регистрации клиента", type: "text", placeholder: "г. Москва, ул. Ленина, д. 1, кв. 1", optional: true },
    { key: "performer", label: "Ваши данные (наименование / ФИО, ИНН)", type: "text", placeholder: "ИП Смирнова М.В., ИНН 770000000000", autofill: "performer" },
    { key: "purpose", label: "Цель обработки данных", type: "text", placeholder: "Оказание услуг психологического консультирования" },
    { key: "phone", label: "Ваш контактный номер телефона для отзыва согласия", type: "tel", placeholder: "9001234567", hint: "Только цифры, до 10 знаков. Например: 9001234567", autofill: "phone" },
    { key: "email", label: "Ваш контактный E-mail для отзыва согласия", type: "email", placeholder: "mail@example.ru", autofill: "email" },
    { key: "signDate", label: "Дата подписания", type: "date" },
  ],
  build: (v) => {
    const ident = v.identMode === "passport"
      ? `паспорт ${or(v.passport, "[серия, номер]")}`
      : "паспортные данные не указываются при электронной подписи";
    const addr = v.address ? `, адрес регистрации: ${v.address}` : "";

    return `Я, ${or(v.fio, "[ФИО]")}${v.birth ? `, ${fmt(v.birth)} г.р.` : ""}, ${ident}${addr}, в соответствии с Федеральным законом от 27.07.2006 № 152-ФЗ «О персональных данных» даю ${or(v.performer, "[наименование/ФИО исполнителя, ИНН]")} согласие на обработку моих персональных данных.

1. Цель обработки: ${or(v.purpose, "[цель обработки]")}.
2. Перечень персональных данных: ФИО, дата рождения, паспортные данные (при указании), адрес, контактный телефон, адрес электронной почты и иные данные, предоставленные мной при обращении.
3. Перечень действий с персональными данными: сбор, запись, систематизация, накопление, хранение, уточнение, использование, передача (предоставление) в объёме, необходимом для оказания услуг, обезличивание, блокирование, удаление, уничтожение.
4. Способ обработки: с использованием средств автоматизации и без использования таких средств.
5. Срок действия согласия: до момента достижения целей обработки либо до отзыва согласия, в зависимости от того, что наступит раньше.

Согласие может быть отозвано в любой момент путём направления письменного уведомления посредством СМС на номер телефона ${or(v.phone, "[номер]")} и/или на электронный адрес ${or(v.email, "[e@mail]")}.
